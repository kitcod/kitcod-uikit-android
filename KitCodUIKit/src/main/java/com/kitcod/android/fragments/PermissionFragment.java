...
import com.kitcod.android.utils.PermissionUtils;
        ...
        if (requestCode == this.requestCode && grantResults.length == requested.length) {
            ...
            if (isAllGranted) {
                ...
                if (deniedList != null && deniedList.size() == 0) {
                    ...
                    builder.setTitle(getActivity().getString(R.string.kc_text_dialog_permission_title));
                    builder.setMessage(getPermissionGuideMessage(getActivity(), notGranted[0]));
                    if (icon != null) {
                        builder.setIcon(icon);
                    }
                    builder.setPositiveButton(R.string.kc_text_go_to_settings, (dialogInterface, i) -> {
    ...
    private static String getPermissionGuideMessage(@NonNull Context context, @NonNull String permission) {
        ...
        switch (permission) {
            case Manifest.permission.CAMERA:
                textResId = R.string.kc_text_need_to_allow_permission_camera;
                break;
            default:
                textResId = R.string.kc_text_need_to_allow_permission_storage;
                break;

        }
        return String.format(Locale.US, context.getString(textResId), "");
    }
}
